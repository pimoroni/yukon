...
- `gamepad_mappings.py` holds the report layout of each known pad and mode, one function each: the
  8BitDo Lite, the 8BitDo SN30 Pro+ in each of its four modes, and the 8BitDo SN30 in its Switch
  mode. Set `PAD_MAPPING` in `main.py` to the one in use. 8BitDo pads pick their mode by the button
